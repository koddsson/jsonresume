...
import {putFile, getMetadata} from './utils/files.js'

import json from 'ajv/lib/refs/json-schema-draft-06.json' assert { type: 'json' };
...
export default async (req, res) => {
  ...
  if (!req.headers.auth) {
    return res.status(400).json({
      errors: ['You need to supply a password via the `auth` header on first upload.']
    })
  }
  ...
  if (metadata) {
    if (req.headers.auth !== decrypt(metadata.auth)) {
      return res.status(401).json({
        errors: ["The auth header does not match our records. Please make sure it's correct."]
      })
    }
  }
  ...
  Promise.all([
    ...
    putFile(`${username}.json`, JSON.stringify(req.body), {auth}, 'application/json')
  ])
    .then(() => {
      // Success!
